...
def EM(data, init_means, init_covariances, init_weights, maxiter=1000, thresh=1e-4):
    ...
    for i in range(maxiter):
        ...
        for k in range(num_clusters):
            ...
            # YOUR CODE HERE
            weights[k] = counts[k] * 1.0 / num_data
            ...
            # YOUR CODE HERE
            means[k] = weighted_sum * 1.0 / counts[k]
            ...
            # YOUR CODE HERE
            covariances[k] = weighted_sum * 1.0 / counts[k]
...
print "data[:5]: "; print data[:5]
print "len(data[0]): %d" % (len(data[0]))

print "\ntype(initial_means): %s" % (type(initial_means))
print "len(initial_means): %d" % (len(initial_means))
print "initial_means: "; print initial_means

print "\ntype(initial_covs): %s" % (type(initial_covs))
print "len(initial_covs): %d" % (len(initial_covs))
print "initial_covs: "; 
for ix in xrange(len(initial_covs)):
    print "type(element in initial_covs): %s" % (type(initial_covs[ix]))
    print initial_covs[ix]
...
# Your code here
# print results['covs']
for ix in xrange(len(results['covs'])):
    print "type(element in results['covs']): %s" % (type(results['covs'][ix]))
    print results['covs'][ix]
