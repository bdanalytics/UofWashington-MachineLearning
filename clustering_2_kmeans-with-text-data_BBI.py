...
sttTm = time.time()
tf_idf, map_index_to_word = sframe_to_scipy(glbObs, 'tf_idf')
...
dist = pairwise_distances(tf_idf, tf_idf[0:3, :], metric='euclidean')[430, 1]
print dist
...
def visualize_document_clusters(glbObs, tf_idf, centroids, cluster_assignment, k, map_index_to_word, display_content=True):
    '''glbObs: original dataframe
    ...
    for c in xrange(k):
        ...
        if display_content:
            ...
            for i in xrange(8):
                text = ' '.join(glbObs[nearest_neighbors[i]]['text'].split(None, 25)[0:25])
                print('\n* {0:50s} {1:.5f}\n  {2:s}\n  {3:s}'.format(glbObs[nearest_neighbors[i]]['name'],
...
visualize_document_clusters(glbObs, tf_idf, centroids[2](), cluster_assignment[2](), 2, map_index_to_word)
...
k = 10
visualize_document_clusters(glbObs, tf_idf, centroids[k](), cluster_assignment[k](), k, map_index_to_word)
...
visualize_document_clusters(glbObs, tf_idf, centroids[25](), cluster_assignment[25](), 25,
...
k=100
visualize_document_clusters(glbObs, tf_idf, centroids[k](), cluster_assignment[k](), k,
...
# print type(cluster_assignment[100]())
# print cluster_assignment[100]().shape
cluster_100_counts = np.bincount(cluster_assignment[100]())
print cluster_100_counts
# np.histogram(cluster_100_counts)
plt.hist(cluster_100_counts, bins = [0, 236, 500, 1000, 2000, 5000, 10000])
plt.show()
